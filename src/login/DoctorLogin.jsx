...
const DoctorLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (userDoc.exists()) {
        alert(`Logged in as Doctor: ${user.email}`);
        navigate('/Login'); // Redirect to doctor's dashboard
      } else {
        alert('Doctor account not found. Please check your credentials.');
      }
    } catch (error) {
      console.error('Error logging in as Doctor:', error);
      alert('Failed to log in as Doctor. Please check your credentials.');
    }
  };
  ...
  return (
    ...
    <div
      ...
      <div className="bg-[#232946] p-6 rounded-lg shadow-md max-w-md w-full bg-opacity-90">
        <h2 className="text-2xl font-bold mb-6 text-center text-[#b8c1ec]">Doctor Login</h2>
